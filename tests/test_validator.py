...
@pytest.mark.asyncio
async def test_staleness_warning_for_old_state(hass: HomeAssistant):
    """Test that validator warns about states not seen recently."""
    ...
    # Simulate that "on" state was seen 60 days ago
    kb._observed_states["binary_sensor.motion"] = {"on", "off"}
    kb._last_seen["binary_sensor.motion"] = {
        "on": datetime.now(timezone.utc) - timedelta(days=60),
        "off": datetime.now(timezone.utc) - timedelta(hours=1),
    }

    # Set threshold to 30 days
    validator = ValidationEngine(kb, staleness_threshold_days=30)
    ...
    assert len(issues) == 1
    assert issues[0].issue_type == IssueType.STALE_STATE
    assert issues[0].severity == Severity.WARNING
    assert "hasn't been seen" in issues[0].message.lower()


@pytest.mark.asyncio
async def test_no_staleness_warning_for_recent_state(hass: HomeAssistant):
    """Test that validator doesn't warn about recently seen states."""
    from datetime import datetime, timedelta, timezone
    ...
    # Simulate that "on" state was seen 10 days ago (within threshold)
    kb._observed_states["binary_sensor.motion"] = {"on", "off"}
    kb._last_seen["binary_sensor.motion"] = {
        "on": datetime.now(timezone.utc) - timedelta(days=10),
        "off": datetime.now(timezone.utc) - timedelta(hours=1),
    }
...
@pytest.mark.asyncio
async def test_no_staleness_check_when_disabled(hass: HomeAssistant):
    """Test that staleness check is skipped when threshold is None."""
    from datetime import datetime, timedelta, timezone
    ...
    # Staleness checking disabled (None)
    validator = ValidationEngine(kb, staleness_threshold_days=None)
    ...
    # No staleness warning because feature is disabled
    assert len(issues) == 0
