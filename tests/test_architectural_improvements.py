...
async def test_async_load_history_filters_to_whitelisted_domains():
    ...
    # Set up mock entities across various domains
    mock_states = []
    for eid in [
        ...
        "group.all_lights",
        "sensor.temperature",      # NOT in whitelist
        "light.living_room",       # NOT in whitelist
        "switch.kitchen",          # NOT in whitelist
        "climate.thermostat",      # NOT in whitelist
    ]:
        s = MagicMock()
        s.entity_id = eid
        mock_states.append(s)
    mock_hass.states.async_all.return_value = mock_states

    kb = StateKnowledgeBase(mock_hass)

    # Capture the entity_ids passed to get_significant_states
    captured_entity_ids = None

    def fake_get_significant_states(hass, start, end, entity_ids, *args, **kwargs):
        nonlocal captured_entity_ids
        captured_entity_ids = entity_ids
        return {}

    # Make async_add_executor_job call the function synchronously and return result
    async def fake_executor_job(func, *args):
        return func(*args)

    mock_hass.async_add_executor_job = fake_executor_job
    ...
    assert captured_entity_ids is not None
    # Only whitelisted domain entities should be included
    captured_set = set(captured_entity_ids)
    for eid in captured_entity_ids:
        domain = eid.split(".")[0]
        assert domain in STATE_VALIDATION_WHITELIST, (
            f"{eid} has domain '{domain}' not in whitelist"
        )
    # Verify whitelisted entities ARE included
    assert "binary_sensor.motion" in captured_set
    assert "person.john" in captured_set
    assert "sun.sun" in captured_set
    # Verify non-whitelisted entities are NOT included
