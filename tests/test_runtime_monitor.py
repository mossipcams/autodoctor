...
def test_ecod_detector_fits_training_rows_and_scores_current_row() -> None:
    """ECOD detector should fit on training rows and score only the current row."""
    from custom_components.autodoctor.runtime_monitor import _EcodAnomalyDetector

    detector = _EcodAnomalyDetector()
...
def test_ecod_detector_refits_on_each_call() -> None:
    """ECOD detector should fit on full training rows each scoring call."""
    from custom_components.autodoctor.runtime_monitor import _EcodAnomalyDetector
    ...
    first_fit_rows = len(mock_model.fit.call_args_list[0].args[0])
    second_fit_rows = len(mock_model.fit.call_args_list[1].args[0])
    assert first_fit_rows == 9
    assert second_fit_rows == 10
...
@pytest.mark.asyncio
async def test_validate_automations_logs_ecod_unavailable(
    hass: HomeAssistant, caplog: pytest.LogCaptureFixture
) -> None:
    """Should log when ECOD detector is unavailable."""
    monitor = RuntimeHealthMonitor(hass, detector=None)
    # Force detector to None to simulate ECOD unavailable
    ...
    assert "ECOD detector unavailable" in caplog.text
...
def test_ecod_detector_logs_model_fit(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """ECOD detector should log when fitting a model."""
    from custom_components.autodoctor.runtime_monitor import _EcodAnomalyDetector
    ...
    with (
        ...
        patch("custom_components.autodoctor.runtime_monitor.ECOD") as mock_ecod,
    ):
        mock_ecod.return_value = mock_model
        detector.score_current("auto.test", rows)

    assert "Fitting ECOD model for 'auto.test' on 2 rows" in caplog.text


def test_ecod_detector_handles_window_changes(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """ECOD detector should work when history shrinks between calls."""
    from custom_components.autodoctor.runtime_monitor import _EcodAnomalyDetector
...
def test_ecod_detector_logs_score(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """ECOD detector should log score result."""
    from custom_components.autodoctor.runtime_monitor import _EcodAnomalyDetector
