...
def test_bocpd_update_state_property_normalized_with_mixed_numeric_counts(
    counts: list[float],
) -> None:
    """Update path should stay stable for mixed numeric/invalid observed counts."""
    detector = BOCPDDetector(hazard_rate=0.05, max_run_length=32)
    state = detector.initial_state()

    for observed in counts:
        detector.update_state(state, observed)  # type: ignore[arg-type]
        probs = state["run_length_probs"]
        assert probs
        assert all(0.0 <= value <= 1.0 for value in probs)
        assert sum(probs) == pytest.approx(1.0, abs=1e-9)
        assert len(probs) <= detector.max_run_length + 1
        assert len(state["observations"]) <= detector.max_run_length


@given(
    run_length_probs=st.lists(
        st.one_of(
...
def test_bocpd_normalize_state_property_handles_malformed_payload(
    run_length_probs: list[object],
    observations: list[object],
    current_count: object,
) -> None:
    """Normalization should make malformed persisted payloads safe and bounded."""
    detector = BOCPDDetector(hazard_rate=0.05, max_run_length=32)
    state = {
...
@given(
    baseline_count=st.integers(min_value=1, max_value=100),
    surge_count=st.integers(min_value=101, max_value=200000),
    history_days=st.integers(min_value=8, max_value=90),
)
@settings(max_examples=120)
def test_bocpd_score_current_property_bounded_and_finite(
    baseline_count: int,
    surge_count: int,
    history_days: int,
) -> None:
    """Score should remain finite and on the detector's bounded anomaly scale."""
    detector = BOCPDDetector(hazard_rate=0.05, max_run_length=90)

    baseline = [
        {
            "rolling_24h_count": float(baseline_count),
            "rolling_7d_count": float(baseline_count * 7),
            "hour_ratio_30d": 1.0,
            "gap_vs_median": 1.0,
            "is_weekend": 0.0,
            "other_automations_5m": 0.0,
    ...
    score = detector.score_current(
        ...
        [
            *baseline,
            {
                "rolling_24h_count": float(surge_count),
                "rolling_7d_count": float(surge_count * 7),
                "hour_ratio_30d": 5.0,
                "gap_vs_median": 1.0,
                "is_weekend": 0.0,
                "other_automations_5m": 0.0,
