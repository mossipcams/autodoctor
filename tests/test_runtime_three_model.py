...
async def test_backfill_from_recorder_seeds_three_models_from_history(
    ...
    assert kitchen_state["gap_model"]["last_trigger"] == history["automation.kitchen"][-1].isoformat()
    assert hallway_state["gap_model"]["last_trigger"] == history["automation.hallway"][-1].isoformat()
