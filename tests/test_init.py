...
async def test_async_setup_entry_runs_runtime_rebuild_from_store() -> None:
    ...
    with (
        ...
        patch(
            "custom_components.autodoctor._get_automation_configs",
            return_value=automation_configs,
        ),
        patch(
            "custom_components.autodoctor._async_register_card", new_callable=AsyncMock
        ),
...
@pytest.mark.asyncio
async def test_setup_runtime_gap_listener_reconciles_cached_runtime_issues() -> None:
    """Gap check callback should sync runtime alerts into cached issue surfaces."""
    from custom_components.autodoctor import _setup_runtime_gap_check_listener

    non_runtime_issue = ValidationIssue(
        issue_type=IssueType.ENTITY_NOT_FOUND,
        severity=Severity.ERROR,
        automation_id="automation.non_runtime",
        automation_name="Non Runtime",
    ...
    stale_runtime_issue = ValidationIssue(
        ...
        entity_id="automation.old_runtime",
    ...
    active_runtime_issue = ValidationIssue(
        ...
        entity_id="automation.new_runtime",
        location="runtime.health.gap",
        message="New runtime alert",
    )
    ...
    def _group_payload(issue: ValidationIssue) -> dict[str, object]:
        return {"issues": [issue], "duration_ms": 5}
    ...
    hass.data = {
        DOMAIN: {
            ...
            "issues": [non_runtime_issue, stale_runtime_issue],
            "validation_issues": [non_runtime_issue, stale_runtime_issue],
            "validation_issues_raw": [non_runtime_issue, stale_runtime_issue],
            "validation_groups": {
                "templates": {"issues": [], "duration_ms": 0},
                "services": {"issues": [], "duration_ms": 0},
                "entity_state": _group_payload(non_runtime_issue),
                "runtime_health": _group_payload(stale_runtime_issue),
            },
    ...
    runtime_monitor = MagicMock()
    runtime_monitor.check_gap_anomalies = MagicMock(return_value=[active_runtime_issue])
    runtime_monitor.get_active_runtime_alerts = MagicMock(
        return_value=[active_runtime_issue]
    )
    ...
    runtime_monitor.check_gap_anomalies.assert_called_once()
    runtime_monitor.get_active_runtime_alerts.assert_called_once()
    reporter.async_report_issues.assert_awaited_once()

    cached_raw = hass.data[DOMAIN]["validation_issues_raw"]
    assert non_runtime_issue in cached_raw
    assert active_runtime_issue in cached_raw
    assert stale_runtime_issue not in cached_raw

    cached_visible = hass.data[DOMAIN]["validation_issues"]
    assert non_runtime_issue in cached_visible
    assert active_runtime_issue in cached_visible
    assert stale_runtime_issue not in cached_visible

    runtime_group_raw = hass.data[DOMAIN]["validation_groups_raw"]["runtime_health"][
        "issues"
    ]
    runtime_group_visible = hass.data[DOMAIN]["validation_groups"]["runtime_health"][
        "issues"
    ]
    assert runtime_group_raw == [active_runtime_issue]
    assert runtime_group_visible == [active_runtime_issue]


@pytest.mark.asyncio
async def test_async_setup_entry_registers_location_cache_invalidation_listeners() -> (
    None
):
    """Setup should register zone and area listeners to invalidate KB location caches."""
    from custom_components.autodoctor import async_setup_entry
    ...
    entry = MagicMock()
    entry.options = {"validate_on_reload": False, "runtime_health_enabled": False}
    entry.add_update_listener = MagicMock(return_value=None)
    entry.async_on_unload = MagicMock()
    ...
    with (
        patch("custom_components.autodoctor.StateKnowledgeBase", return_value=mock_kb),
        patch("custom_components.autodoctor.SuppressionStore") as mock_suppression_cls,
        patch("custom_components.autodoctor.LearnedStatesStore") as mock_learned_cls,
        patch(
            "custom_components.autodoctor._async_register_card", new_callable=AsyncMock
    ...
    assert EVENT_STATE_CHANGED in listener_callbacks
    assert ar.EVENT_AREA_REGISTRY_UPDATED in listener_callbacks

    zone_listener = listener_callbacks[EVENT_STATE_CHANGED]
    assert callable(zone_listener)
    zone_event = MagicMock()
    zone_event.data = {"entity_id": "zone.home"}
    zone_listener(zone_event)
    mock_kb.invalidate_location_caches.assert_called_once()

    unrelated_event = MagicMock()
    unrelated_event.data = {"entity_id": "light.kitchen"}
    zone_listener(unrelated_event)
    assert mock_kb.invalidate_location_caches.call_count == 1

    area_listener = listener_callbacks[ar.EVENT_AREA_REGISTRY_UPDATED]
    assert callable(area_listener)
    area_listener(MagicMock())
    assert mock_kb.invalidate_location_caches.call_count == 2


@pytest.mark.asyncio
async def test_unload_entry_calls_location_cache_unsub_listeners() -> None:
    """Unload should unsubscribe zone/area location cache invalidation listeners."""
    from custom_components.autodoctor import async_unload_entry

    hass = MagicMock()
    entry = MagicMock()
    unsub_zone_listener = MagicMock()
    unsub_area_listener = MagicMock()
    ...
    hass.data = {
        DOMAIN: {
            ...
            "unsub_reload_listener": None,
            "unsub_entity_registry_listener": None,
            "unsub_zone_state_listener": unsub_zone_listener,
            "unsub_area_registry_listener": unsub_area_listener,
        }
    }
