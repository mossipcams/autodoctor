...
class RuntimeHealthMonitor:
    ...
    def _build_overdue_profile(
        ...
        *,
        ...
        comparable_days = sum(
            1
            for day_offset in range(max(0, (now.date() - baseline_start.date()).days))
            if (baseline_start.date() + timedelta(days=day_offset)).weekday()
            == now.weekday()
        )
        comparable_events = [
    ...
    def _predict_overdue(
        ...
        *,
        ...
        if self.baseline_days < 90 or observed_days < 90:
            return {
                "status": "abstain",
        ...
        for ts in comparable_events:
            comparable_by_day[ts.date()].append(ts)
        first_trigger_minutes = sorted(
            float(first.hour * 60 + first.minute + (first.second / 60.0))
            for first in (min(values) for values in comparable_by_day.values())
        )
        active_comparable_days = len(first_trigger_minutes)
