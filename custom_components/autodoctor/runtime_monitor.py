...
class _IsolationForestDetector:
    """IsolationForest-backed detector using sklearn (ships with HA)."""
    ...
    def score_current(
        ...
        _LOGGER.debug(
            "Fitting IsolationForest for '%s' on %d rows",
            automation_id,
            len(training),
        )
...
class RuntimeHealthMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self._detector = detector or (
            _IsolationForestDetector() if _HAS_SKLEARN else None
        )
        ...
        _LOGGER.debug(
            ...
            "sklearn_available=%s, detector=%s",
            baseline_days,
            warmup_samples,
            anomaly_threshold,
            min_expected_events,
    ...
    async def validate_automations(
        ...
        if self._detector is None:
            _LOGGER.debug(
                "Detector unavailable, skipping %d automations",
                len(automations),
            )
            stats["detector_unavailable"] = len(automations)
