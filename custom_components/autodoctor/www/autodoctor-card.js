 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function pt(t){return ht({...t,state:!0,attribute:!1})}var gt;console.info("%c AUTODOCTOR-CARD %c 2.1.0 ","color: white; background: #3498db; font-weight: bold;","color: #3498db; background: white; font-weight: bold;");let ft=gt=class extends nt{constructor(){super(...arguments),this._loadingValidation=!0,this._error=null,this._activeTab="validation",this._validationData=null,this._runningValidation=!1,this._dismissedSuggestions=new Set,this._validationRequestId=0,this._suppressionInProgress=!1,this._lastValidationClick=0}_isInCooldown(){return Date.now()-this._lastValidationClick<gt.CLICK_COOLDOWN_MS}setConfig(t){this.config=t}static getStubConfig(){return{type:"custom:autodoctor-card"}}static async getConfigElement(){return await Promise.resolve().then(function(){return vt}),document.createElement("autodoctor-card-editor")}async firstUpdated(){await this._fetchValidation()}async _fetchValidation(){const t=++this._validationRequestId;this._loadingValidation=!0;try{this._error=null;const e=await this.hass.callWS({type:"autodoctor/validation"});t===this._validationRequestId&&(this._validationData=e)}catch(e){t===this._validationRequestId&&(console.error("Failed to fetch validation data:",e),this._error="Failed to load validation data")}t===this._validationRequestId&&(this._loadingValidation=!1)}async _runValidation(){if(this._runningValidation||this._isInCooldown())return;this._lastValidationClick=Date.now();const t=++this._validationRequestId;this._runningValidation=!0;try{const e=await this.hass.callWS({type:"autodoctor/validation/run"});t===this._validationRequestId&&(this._validationData=e)}catch(e){t===this._validationRequestId&&console.error("Failed to run validation:",e)}t===this._validationRequestId&&(this._runningValidation=!1)}_groupIssuesByAutomation(t){const e=new Map;for(const s of t){const{issue:t,edit_url:i}=s,o=t.automation_id;e.has(o)||e.set(o,{automation_id:t.automation_id,automation_name:t.automation_name,issues:[],edit_url:i,has_error:!1,error_count:0,warning_count:0});const r=e.get(o);r.issues.push(s),"error"===t.severity?(r.has_error=!0,r.error_count++):r.warning_count++}return Array.from(e.values())}_getCounts(t){if(!t)return{errors:0,warnings:0,healthy:0,suppressed:0};let e=0,s=0;for(const i of t.issues)"error"===i.issue.severity?e++:s++;return{errors:e,warnings:s,healthy:t.healthy_count,suppressed:t.suppressed_count||0}}get _loading(){return this._loadingValidation}render(){const t=this.config.title||"Autodoctor";if(this._loading)return this._renderLoading(t);if(this._error)return this._renderError(t);const e=this._validationData;if(!e)return this._renderEmpty(t);const s=this._groupIssuesByAutomation(e.issues),i=this._getCounts(e),o=e.issues.length>0;return q`
      <ha-card>
    ...
    `}_renderTabFooter(){const t=this._runningValidation||this._loadingValidation||this._isInCooldown(),e=this._validationData?.last_run,s=this._runningValidation||this._loadingValidation;return q`
      <div class="footer">
        <button
        ...
        >
          ...
          <span class="run-text">${s?"Running...":"Run Validation"}</span>
        </button>
        ${e?q` <span class="last-run">Last run: ${this._formatLastRun(e)}</span> `:W}
      </div>
    `}_formatLastRun(t){const e=new Date(t),s=(new Date).getTime()-e.getTime(),i=Math.floor(s/6e4);if(i<1)return"just now";if(i<60)return`${i}m ago`;const o=Math.floor(i/60);if(o<24)return`${o}h ago`;return`${Math.floor(o/24)}d ago`}_getSuggestionKey(t){return`${t.automation_id}:${t.entity_id}:${t.message}`}_dismissSuggestion(t){const e=this._getSuggestionKey(t);this._dismissedSuggestions=new Set([...this._dismissedSuggestions,e])}async _suppressIssue(t){if(!this._suppressionInProgress){this._suppressionInProgress=!0;try{await this.hass.callWS({type:"autodoctor/suppress",automation_id:t.automation_id,entity_id:t.entity_id,issue_type:t.issue_type||"unknown"}),await this._fetchValidation()}catch(t){console.error("Failed to suppress issue:",t)}finally{this._suppressionInProgress=!1}}}async _clearSuppressions(){if(!this._suppressionInProgress){this._suppressionInProgress=!0;try{await this.hass.callWS({type:"autodoctor/clear_suppressions"}),await this._fetchValidation()}catch(t){console.error("Failed to clear suppressions:",t)}finally{this._suppressionInProgress=!1}}}_renderAutomationGroup(t){return q`
      <div class="automation-group ${t.has_error?"has-error":"has-warning"}">
        ...
        <div class="automation-issues">${t.issues.map(t=>this._renderIssue(t))}</div>
    ...
    `}};t([ht({attribute:!1})],mt.prototype,"hass",void 0),t([pt()],mt.prototype,"_config",void 0),mt=t([ct("autodoctor-card-editor")],mt);var vt=Object.freeze({__proto__:null,get AutodoctorCardEditor(){return mt}});export{ft as AutodoctorCard};
